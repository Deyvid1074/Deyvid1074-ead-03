...
function iniciar() {
  ...
  chkTituloItalico.addEventListener('change', habilitarTituloItalico);

  var chkInverterDiaMes = document.querySelector('#chkInverterDiaMes');
  chkInverterDiaMes.addEventListener('change', habilitarInverterDiaMes);
}
...
function habilitarTituloItalico() {
  var classe1 = 'textoItalico';
  var cabecalho = document.querySelector('#cabecalhoBanda');  

  //Aqui, this é o checkbox!
  if(this.checked) {
    cabecalho.classList.add(classe1);
  }
  else {
    cabecalho.classList.remove(classe1);    
  }
}

function habilitarInverterDiaMes() {
